...
| `num` | `min max abs clamp pow rem gcd toText toFloat round truncate` | accepted as explicit importable source module via `import stdlib.num`; exports `min` `max` `abs` `clamp` `pow` `rem` `gcd` `toText` `toFloat` `round` `truncate`; `toText` is a pure source-canonical full-range signed decimal renderer; other Int helpers are source wrappers over checked scalar bridge intrinsics where needed; `toFloat`/`round`/`truncate` are conversion intrinsics; interpreter/TLC/native parity covered |
