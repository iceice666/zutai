...
- Design decisions
  - [0001 — Record update and config layering](decisions/0001-record-update-and-config-layering.md)
  - [0002 — Interim THIR reference interpreter](decisions/0002-interim-thir-interpreter.md)
...
- **Reference interpreter** (`zutai-eval`) — interim THIR tree-walking interpreter. A semantics oracle: runs only fully type-checked `.zt` programs; provides `run` and `repl` CLI subcommands; output is ground truth for differential testing of the LLVM backend. See [decision 0002](decisions/0002-interim-thir-interpreter.md).
