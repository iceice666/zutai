...
v1 inherits v0 surface syntax entirely. Only new semantic constructs are introduced: row tails (`...`, `...Rest`), `@T` witness targets, `!` effect rows, `derive`, `select`, `fields`, and `schema`. Top-level typed functions and constraint-method defaults use `= pat => body;` clauses after the signature. `match` arms use `{ | pat => body; }` clause blocks. Type parameters use `<A>`, exactly as in v0.
