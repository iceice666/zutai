...
Witnesses are automatically exported when their defining module is imported. There are no orphan restrictions in Zutai, but implementations may warn when a witness is defined outside both the constraint's module and the type's module.
...
Higher-kinded constraints target constructors of kind `Type -> Type`.
Constructors of higher arity may be partially applied until they have that
kind; higher-order kind targets are an explicit non-goal.
...
The compiler supplies the structural equality recipe for a constraint marked
`derive`. A constraint may also attach its own compile-time recipe using
reflection and `witness`; see
