...
Zutai supports higher-rank polymorphism—quantifiers nested inside a function's
argument types—through explicit annotations and bidirectional checking.
Inference remains predicative and rank-1.
...
principal, is a type error asking for an annotation — consistent with Zutai's
inference-boundary policy (see [row polymorphism](../06-polymorphism/row-polymorphism.md)).
...
Zutai higher-rank polymorphism stays **predicative**: a type variable is never
...
is outside Zutai. Impredicative instantiation—placing a quantified type where a
monotype is expected—is an explicit non-goal. Rank is unbounded in
