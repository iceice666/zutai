...
Status: stable syntax with interpreter and native support in the documented
envelope. The initial generator shell introduced finite streams; the current
`Stream A` representation is demand-driven codata, and `yield` appears under
conditionals and recursion, so pure finite *and* infinite generators
type-check and evaluate on both the interpreter and the native backend. Effectful
generators run under a granting handler. **Native effect parity landed
...
- **Open question settled: richer `yield` is statement syntax desugared by
  continuation-passing onto the codata cell, not handler sugar.** A
...
- **Cancellation landed** (generator support) as consumer-driven mid-stream
...
- No *preemptive/asynchronous* cancellation runtime. Cooperative cancellation —
  ...
  abort + `finally` machinery (see "Cancellation"). Resource
...
Pure infinite/recursive generators *are* supported through codata and richer
`yield`: `range lo hi = stream { if lo < hi then { yield lo; yield from
