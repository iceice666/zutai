...
Zutai already treats types as first-class compile-time values and allows pure type-level computation with deterministic evaluator limits. This chapter records Zutai baseline and Zutai extensions that build on it, such as richer type functions, explicit kind annotations, and normalization guidance.

---

## Zutai Baseline: `Type` Values

Types are first-class compile-time values. A `:: type` alias or a binding
annotated `:: Type` holds a type:
...
See [universe levels](../05-type-system/universe-levels.md) for Zutai stratification design.
