...
A `type_expr` is a type-level expression. In Zutai, type expressions are pure expressions that evaluate to `Type`, including type names, type applications, optional types (`T?`), record types (`{ ... }`), tagged union types (`{ #tag; ... }`), and list types (`List T`).
...
Record typing uses closed records in Zutai. See [Record types](records.md).
