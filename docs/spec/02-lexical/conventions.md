...
In Zutai, doc comments are lexically distinct from ordinary line comments, but they have no required semantic effect.
...
- Without a postfix, existing Zutai inference is unchanged: an integer-looking body has type `Int`; a body with a `.` or an exponent has type `Float`.
...
`Int` is the default signed integer type and is an alias of `i64` in Zutai.

`Float` is the default floating-point type and is an alias of `f64` in Zutai.
...
`Number` is an optional host/prelude convenience rather than a required builtin.
...
The following words are reserved because they introduce stable syntax:
...
| `#`              | atom prefix                                                                |
| `+` `-` `*` `/` `%` | arithmetic operators; `*` also introduces value/type/effect spreads    |
| `==` `!=` `<` `<=` `>` `>=` | comparison operators                                            |
| `&&` `\|\|`      | logical AND / OR (short-circuit); operands and result are `Bool`           |
| `...`            | anonymous or named row tail in record, union, and effect-row types         |
| `!`              | effect-row marker on types; punctuation spelling of `perform`              |
| `^`              | punctuation spelling of `resume`                                           |
| `>>=`            | selective projection operator                                              |
...
There is no unary operator in Zutai: negation is part of a numeric literal (e.g. `-10`, `x * -1`).
