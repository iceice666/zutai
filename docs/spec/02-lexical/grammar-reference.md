...
Postfix
  ::= FieldSection
   | AtomExpr (("." | "?.") FieldName | RecordUpdate)*
...
GenStmt
  ::= "yield" Expr ";"
   | "yield" "from" Expr ";"
   | "if" Expr "then" "{" GenStmt* "}" ("else" "{" GenStmt* "}")?
...
TypeRecord
  ::= "{" (TypeRecordField | RowSpread)* RowTail? "}"

TypeRecordField
  ::= FieldName "?"? ":" TypeExpr ";"

TypeUnion
  ::= "{" (UnionVariant | RowSpread | RowTail)+ "}"
...
Reserved words are not identifiers: `type`, `match`, `if`, `cond`, `then`,
`else`, `import`, `true`, `false`, `select`, `perform`, `handle`, `with`, and
`resume`. `derive`, `stream`, `yield`, `from`, `witness`, `fields`, `variants`,
and `schema` are contextual: they retain their special meaning only in the
grammar positions that introduce those forms.
