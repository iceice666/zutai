...
[Algebraic effects](algebraic-effects.md) split an effectful computation into two
parts: the effect row records *what* may happen, and a capability value records
*who authorized it*. `io.print` is ambient for compatibility; filesystem,
environment, clock, randomness, dynamic loading, and networking operations
...
values only from the entry boundary, never by construction. The capability
parameter is advisory—its presence in the signature
...
`io.print` remains available ambiently for source compatibility;
...
Zutai defines a standard capability set. Each row lists the operation, its standard
...
Capability authority is *advisory*: a capability value is ordinary data, and
...
Zutai does **not** make capabilities unforgeable. Cryptographically or
type-theoretically unforgeable capability tokens, and a typing rule that binds a
specific capability *value* to authority over a specific operation, are an
explicit non-goal. The pure core still has no ambient host access of any kind
beyond `io.print`.
