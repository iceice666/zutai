...
Algebraic effects provide explicit, typed effectful computation while keeping
the language core pure. An operation runs only when code uses `perform`, and it
must be handled or provided by a host boundary.

The effect syntax uses four reserved words:
...
Resumptions are one-shot in Zutai: an operation handler clause may call `resume` at most once.
...
See [host capabilities](host-capabilities.md) for the authorization model and
standard host operation set.
...
implemented Zutai path uses pre-DC handler-passing CPS elaboration:
