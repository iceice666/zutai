...
**Reified handled-effect fallback.** Handler-passing CPS remains the fast path
and still uses no new TLC node. Phase 35 proved that a `Free Op A`-style spine
can lower through recursive Dataflow Core types: cyclic `DfTyId` graphs carry
the same `resume` back-edge shape as guarded recursive user aliases. The native
effect-parity work then scoped that proof into `reify_residual_effects`: a
residual `handle` that is fully reifiable before Dataflow Core is rewritten into
ordinary recursive data, functions, `bind`, and `run` driver declarations. If a
scope is not reifiable, the existing residual-effect gate refuses it rather than
miscompiling. Polymorphic effect execution, open effect rows, unsupported host
operations, and out-of-envelope effectful-generator forms therefore remain
explicit refusal boundaries, not hidden backend effects.
...
- **`perform`/`handle`/`resume` as core nodes.** Effects elaborate before
  Dataflow Core. Lexical handler-passing CPS is the fast path; supported
  residual handled effects use `reify_residual_effects` to generate ordinary
  recursive data/functions plus a driver loop. Do not add core effect nodes for
