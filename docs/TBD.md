...
Open work contains one remaining **v1** milestone that removes an AOT-backend
rejection gate for a construct that already type-checks and evaluates but does
not yet compile natively. The v1 typing and reference-evaluation surface is
otherwise complete — see [`ARCHIVED.md`](ARCHIVED.md) Phases 1–21 and the
archived native-codegen hardening note.
