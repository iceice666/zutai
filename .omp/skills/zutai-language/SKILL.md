...
2. `docs/v0_spec/00-index.md` — normative v0 index. Follow its chapter links for stable syntax and semantics.
3. `docs/v0_spec/02-lexical/grammar-reference.md` — compact implemented general-mode grammar; parser source remains `crates/general/syntax/src/parser/` when code behavior must be verified.
...
6. `docs/v1_spec/00-index.md` — deferred v1 feature index. Use v1 pages only for features explicitly marked implemented or v1-adjacent in the manual/archive.
...
1. Classify the question:
   - Syntax/example question → read `docs/language-manual.md`, then the relevant `docs/v0_spec/` chapter or `grammar-reference.md`.
   ...
   - Deferred/v1 question → read `docs/v1_spec/00-index.md` and the specific v1 page, then cross-check `docs/language-manual.md` and `docs/ARCHIVED.md` before claiming implementation support.
...
- `.zti` immediate mode is inert data: no imports, functions, conditionals, arithmetic, name resolution, type computation, or evaluation. See `docs/language-manual.md` file modes and `docs/v0_spec/01-overview/file-modes.md`.
...
- Do not claim tagged union values serialize to `.zti`/JSON in v0.
- Do not claim full native-backend support for v1-adjacent features unless the current manual/archive says so.
- Do not bypass `docs/v0_spec/` when changing language syntax or semantics.
