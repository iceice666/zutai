    ...
    pub(super) fn lower_expr(&mut self, id: TlcExprId) -> NodeId {
        ...
        match expr {
            ...
            TlcExpr::Letrec { bindings, body } => {
                // TLC→DC makes recursion explicit (see docs/compiler pipeline): the
                // only recursive value the backend realizes is a named global, since
                // ANF's SCC analysis detects self-loops on `GlobalRef` and emits an
                // `AnfDecl::Letrec`. A local recursive binding whose value is a closed
                // lambda (self-reference plus its own params/match-binds — no capture of
                // an enclosing `Bind`) is therefore lifted to a fresh synthetic global.
                // Both the self-call inside the lambda and every outside reference then
                // resolve to the same `GlobalRef`, so the recursion survives to native
                // codegen instead of dropping the self-reference to an `Error` node.
                //
                // The binding name is registered in `local_env` as a `GlobalRef` *before*
                // lowering the value, so the recursive occurrence inside the lambda body
                // resolves to the global rather than falling through to `Error`.
                let lifted: Vec<(BindingId, String)> = bindings
                    .iter()
                    .map(|(binding, _, _)| {
                        (*binding, format!("{}$rec{}", self.module_prefix, binding.0))
                    })
                    .collect();
                ...
                for ((_, _, value_id), (_, name)) in bindings.iter().zip(lifted.iter()) {
                    ...
                    self.insert_global(name.clone(), value_node);
                }
                let body_node = self.lower_expr(body);
