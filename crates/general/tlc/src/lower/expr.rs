...
use crate::ir::{
    BuiltinOp, Literal, PrimTy, Row, TlcAlt, TlcExpr, TlcExprId, TlcHandleClause, TlcPat,
    TlcPatItem, TlcTupleField, TlcTupleItem, TlcType, TlcTypeId, UnresolvedDispatch,
};
...
impl<'thir> Lowerer<'thir> {
    ...
    fn lower_overlay_function_for_target(
        &mut self,
        target: TypeId,
        deep: bool,
        span: Span,
    ...
    fn overlay_partial_apply_parts(
        &self,
        func: ThirExprId,
        result_ty: TypeId,
    ) -> Option<(bool, TypeId)> {
    ...
    fn builtin_overlay_deep(&self, binding: BindingId) -> Option<bool> {
        if !self
            .thir
            .binding_kinds
    ...
    fn lower_optional_nested_overlay(
        &mut self,
